<!--

author:   Stefan Hierholzer
email:    stefan.hierholzer@paedagogika.com
icon:     https://paedagogika-fachschule.de/media/paedagogika-fachschule-weblogo.png
version:  0.0.1
language: de
narrator: Deutsch Male
comment:  Dieses Dokument dient der Begrüßung neuer Studierender
tags:     Neue studierende, Pädagogika, Fachschule, Tutorial


@style

section {
    margin-top: 2rem;
}

iframe {
    min-height: 600px;
}

details {
    padding: 1em;
    border: 1px solid #ccc;
    border-radius: 4px;
    margin-bottom: 0.1em;
    display: block;
}

summary {
    font-weight: bold;
    cursor: pointer;
}

@keyframes details-show {
    from {
        opacity: 0;
        transform: translateY(-0.5em);
    }
}

details[open] > *:not(summary) {
    animation: details-show 500ms ease-in-out;
}

.person-img {
    margin-top: 1em;
    width: 250px;
}

@end

-->

# FH-Workingspace 🧑‍💼 -OpenProject
<div style="width:100%;height:0;padding-bottom:86%;position:relative;"><iframe src="https://giphy.com/embed/payr6u9f4Pa5zYGDid" width="100%" height="100%" style="position:absolute" frameBorder="0" class="giphy-embed" allowFullScreen></iframe></div><p><a href="https://giphy.com/gifs/jojo-joseph-raja-payr6u9f4Pa5zYGDid">via GIPHY</a></p>


# 📂 Was ist das für ein Programm?

![OpenProject-Logo](https://upload.wikimedia.org/wikipedia/commons/d/d5/OpenProjectLogo.png)

OpenProject ist eine webbasierte Projektmanagement-Software, die speziell für Teamzusammenarbeit, Projektplanung und -steuerung sowie für das Fortschrittsmonitoring entwickelt wurde. Es ermöglicht Teams, Projekte effizient zu organisieren und zu verwalten, indem Aufgaben zugewiesen, Zeitpläne erstellt und Ressourcen überwacht werden. OpenProject unterstützt verschiedene Projektmanagement-Methoden, darunter Agile, Scrum und Wasserfall. Die Plattform bietet eine Vielzahl von Funktionen wie Aufgaben- und Zeitmanagement, Produkt-Roadmapping, Bug-Tracking und ein Wiki für Dokumentationen. Dank seiner Open-Source-Natur können Nutzer*innen die Software anpassen und erweitern, um spezifische Bedürfnisse ihres Projekts oder Unternehmens zu erfüllen. Ein weiterer Vorteil ist die Förderung der Transparenz in Projekten, da alle Teammitglieder Zugriff auf Projektinformationen und den Fortschritt haben. OpenProject ermöglicht auch die Integration mit anderen Tools und Plattformen, was die Effizienz und Produktivität weiter steigert. Es ist sowohl für kleine Teams als auch für große Organisationen geeignet und wird weltweit in verschiedenen Branchen eingesetzt. Sicherheitsfunktionen wie rollenbasierte Zugriffskontrolle und regelmäßige Updates sorgen für den Schutz sensibler Daten. Zusammenfassend ist OpenProject eine leistungsfähige Lösung für die Projektmanagementbedürfnisse eines jeden Teams, die Flexibilität, Transparenz und Effizienz in den Vordergrund stellt.

# 🧑‍🏫 Wozu wird es in der Lehre genutzt?
OpenProject ist ein vielseitiges Werkzeug für Projektmanagement und Zusammenarbeit, das an Fachhochschulen sowohl von Dozierenden als auch von Studierenden genutzt werden kann. Dozierende können es verwenden, um Projekte zu planen, Aufgaben zuzuweisen und den Fortschritt der Studierenden zu überwachen. Studierende können es nutzen, um gemeinsam an Projekten zu arbeiten, sich gegenseitig Aufgaben zuzuweisen und Deadlines zu koordinieren. Die Plattform ermöglicht den Austausch von Dateien und die Diskussion in Foren, was die Kommunikation und Kollaboration vereinfacht. Mit der Zeitplanungsfunktion können alle Beteiligten den Überblick über Fristen und Meilensteine behalten. Die Möglichkeit, den Projektfortschritt visuell darzustellen, hilft dabei, Engpässe schnell zu identifizieren und anzugehen. OpenProject fördert somit eine strukturierte Arbeitsweise und unterstützt das selbstorganisierte Lernen. Es bietet eine zentrale Plattform, auf der alle Informationen und Ressourcen gebündelt sind, was die Effizienz und Produktivität steigert. Durch die Nutzung von OpenProject an einer Fachhochschule können Dozierende und Studierende ihre Projekte effektiver und transparenter gestalten.

# 📀 Wie bedienen ich das Programm (Videoanaleitungen)

<div style="width:100%;height:0;padding-bottom:52%;position:relative;"><iframe src="https://giphy.com/embed/ENL5sbMUYeT2L9lDop" width="100%" height="100%" style="position:absolute" frameBorder="0" class="giphy-embed" allowFullScreen></iframe></div><p><a href="https://giphy.com/gifs/snl-saturday-night-live-season-48-ENL5sbMUYeT2L9lDop">via GIPHY</a></p>

## 🧑‍🦱Nutzer*innenperspektive
Zunächst muss sich der*die Nutzer*in auf der Palltform anmelden

**Überblick OpenProjectI**
<iframe width="560" height="315" src="https://www.youtube.com/embed/Auosmd7u13k?si=b5UJXD2x4x9pR7li" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>

**Überblick OpenProject II**
<iframe width="560" height="315" src="https://www.youtube.com/embed/doVtVArSSvk?si=gq1vI8Czm0xG4qvU" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>

> Mit Hilfe des Zahnrads können deutsche Untertitel in die Videos eingebeldnet werden:   https://youtu.be/cvZo3TzTiqA?si=XZlboUWDhuhZLCs8 

**OpenProject add-ons (englisch version)**
<iframe width="560" height="315" src="https://www.youtube.com/embed/OnKl82HqnQk?si=bjGui41y3Qxr7sMB" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>

**OpenProject wiki (englisch version)**
<iframe width="560" height="315" src="https://www.youtube.com/embed/S_piFdI9wDc?si=xdQE5Fk4Wb4M2qcx" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>

**OpenProject Team Planner (englisch version)**
<iframe width="560" height="315" src="https://www.youtube.com/embed/PQknyChahBw?si=exJozKtY2iFsBZgU" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>

**OpenProject Meetings (englisch version)**
<iframe width="560" height="315" src="https://www.youtube.com/embed/xK9m4JJODcc?si=QqZ03ChmRjQjA6yW" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>

**OpenProject Scrum und Agile Boards (englisch version)**
<iframe width="560" height="315" src="https://www.youtube.com/embed/Z7iE_BNCDus?si=lhdNGptN-nCea0eZ" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>



# 📖 Wie bedienen ich das Programm (schriftliche Anleitung) 

<div style="width:100%;height:0;padding-bottom:73%;position:relative;"><iframe src="https://giphy.com/embed/WoWm8YzFQJg5i" width="100%" height="100%" style="position:absolute" frameBorder="0" class="giphy-embed" allowFullScreen></iframe></div><p><a href="https://giphy.com/gifs/cartoons-comics-sea-reading-WoWm8YzFQJg5i">via GIPHY</a></p>

## 🧑‍🦱Nutzer*innenperspektive

https://www.openproject.org/de/docs/nutzungshandbuch/

https://www.tu-chemnitz.de/urz/groupware/tucproject/OpenProject_Grundlagenschulung_klassisch_20211014.pdf 

## 🫅 Administrator*innenperspektive
https://www.openproject.org/de/docs/grundlagen/

# ❣️ Best Practice Beispiel(e)





